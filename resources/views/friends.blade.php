                            ...
                            <ul class="space-y-3">
                                @foreach($friends as $friend)
                                    <li class="flex items-center gap-3 p-3 bg-gray-100 rounded-lg">
                                        <img src="{{ $friend->follower_id == auth()->id() ? $friend->following->profile_picture : $friend->follower->profile_picture}}" 
                                            alt="Profile Image" class="w-12 h-12 rounded-full border border-gray-300">
                                        
                                        <span class="text-gray-800 font-medium text-lg">
                                            {{ $friend->follower_id == auth()->id() ? $friend->following->name : $friend->follower->name }}
                                        </span>
                                    </li>
                                @endforeach
                            </ul>
